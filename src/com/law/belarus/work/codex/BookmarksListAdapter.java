 ...
 */
	public View getView2(final int position,  View convertView, ViewGroup parent) {

		//if (convertView == null)
			convertView = MainActivity.inflater.inflate(R.layout.bookmark_list_item, parent, false);
		
		
		final LinearLayout bookmarkLayout = (LinearLayout)convertView.findViewWithTag(TITLE_LAYOUT_TAG);					//Получаем объект-контейнер текста и кнопки "Удалить"
		final TextView     bookmarkText   = (TextView)bookmarkLayout.findViewWithTag(BOOKMARK_TEXT_TAG);					//Получаем ссылку на текстовое поле для названия статьи
		final TextView     deleteText     = (TextView)convertView.findViewWithTag(DELETE_TAG);							//Получаем ссылку на кнопку "Удалить"
		...
		deleteText.setOnClickListener(new OnClickListener(){
			public void onClick(View v) {
				...
				deleteCell(bookmarkText, position);
